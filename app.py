...
def accountcreation():
    ...
    inserquery('INSERT INTO user (name, email,mno,password,profile) VALUES ("%s", "%s", "%s", "%s","%s")'% \
     (username, email, contact,password,"static/assets/img/default.jpg"))
...
def forgetpass():
    ...
    if(len(user)>0):
        ...
        inserquery('update user set password="%s" where email="%s"' % \
                   (password, email))
